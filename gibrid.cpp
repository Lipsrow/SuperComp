...
void init_mpi(int argc, char* argv[]) {
    ...
    if (mpi_rank == 0) {
        cout << "MPI+OpenMP Hybrid Wave Equation Solver" << endl;
...
void exchange_with_neighbors(Grid3D& grid,
    ...
    if (left != MPI_PROC_NULL) {
        ...
        for (size_t i = 0; i < neighbor_left.size(); i++) {
            neighbor_left[i] = 0.0; 
        }
    }
    ...
    if (right != MPI_PROC_NULL) {
        ...
        #pragma omp parallel for
        for (size_t i = 0; i < neighbor_right.size(); i++) {
            neighbor_right[i] = 0.0;
        }
    }
    ...
    if (bottom != MPI_PROC_NULL) {
        ...
        for (int i = 0; i < local_nx; i++) {
            for (int k = 0; k < local_nz; k++) {
                neighbor_bottom[i * local_nz + k] = grid(i, local_ny-1, k);
            }
        }
    }
    ...
    if (top != MPI_PROC_NULL) {
        ...
        for (int i = 0; i < local_nx; i++) {
            for (int k = 0; k < local_nz; k++) {
                neighbor_top[i * local_nz + k] = grid(i, 0, k);
            }
        }
    }
    ...
    if (back != MPI_PROC_NULL) {
        ...
        for (int i = 0; i < local_nx; i++) {
            for (int j = 0; j < local_ny; j++) {
                neighbor_back[i * local_ny + j] = grid(i, j, local_nz-1);
            }
        }
    }
    ...
    if (front != MPI_PROC_NULL) {
        ...
        for (int i = 0; i < local_nx; i++) {
            for (int j = 0; j < local_ny; j++) {
                neighbor_front[i * local_ny + j] = grid(i, j, 0);
            }
        }
    }
...
double compute_max_error(const Grid3D& numerical, double current_time, double& max_abs_error, double& max_rel_error) {
    ...
    #pragma omp parallel
    {
        double thread_max_abs = 0.0;
        double thread_max_rel = 0.0;
        ...
        for (int i = 0; i < local_nx; i++) {
            for (int j = 0; j < local_ny; j++) {
                for (int k = 0; k < local_nz; k++) {
                    ...
                    if (abs_error > thread_max_abs) {
                        thread_max_abs = abs_error;
                    }
                    if (rel_error > thread_max_rel) {
                        thread_max_rel = rel_error;
                    }
                }
            }
        }
        ...
        #pragma omp critical
        {
            if (thread_max_abs > local_max_abs_error) {
                local_max_abs_error = thread_max_abs;
            }
            if (thread_max_rel > local_max_rel_error) {
                local_max_rel_error = thread_max_rel;
            }
        }
    }
...
void write_time_step_errors_to_file(const vector<int>& steps, const vector<double>& times,
    ...
    write_data_to_file(nullptr, TIME_ERRORS, filename, 0.0, &steps, &times, &errors);
}
...
int main(int argc, char* argv[]) {
    ...
    double local_time = local_end_time - local_start_time;
    double max_local_time, min_local_time, avg_local_time;
    ...
    MPI_Reduce(&local_time, &avg_local_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    avg_local_time /= mpi_size;
    ...
    if (mpi_rank == 0) {
        ...
        write_time_step_errors_to_file(time_steps, time_values, max_abs_errors, "hybrid_time_step_errors.txt");
        cout << "Time step errors saved to: hybrid_time_step_errors.txt" << endl;
    }

    //write_data_to_file(&u1, NUMERICAL, "hybrid_numerical.txt", T);
    //write_data_to_file(&u1, ERROR, "hybrid_error.txt", T);
    //write_data_to_file(nullptr, ANALYTICAL, "hybrid_analytical.txt", T);
