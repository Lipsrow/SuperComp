...
double compute_max_abs_error_at_time_step(const Grid3D& numerical, double current_time) {
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                ...
                for (int k = 0; k < N; k++) {
                    ...
                    local_max_abs = max(local_max_abs, abs_error);
                }
            }
        }
        ...
        #pragma omp critical
        {
            max_abs_error = max(max_abs_error, local_max_abs);
        }
    }
...
void compute_errors(const Grid3D& numerical, const Grid3D& analytical,
    ...
    #pragma omp parallel
    {
        ...
        double local_max_analytical = 0.0;
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                for (int k = 0; k < N; k++) {
                    ...
                    local_max_abs = max(local_max_abs, abs_error);
                    local_max_rel = max(local_max_rel, rel_error);
                    local_rms += abs_error * abs_error;
                    local_max_analytical = max(local_max_analytical, fabs(exact));
                }
            }
        }
        ...
        {
            max_abs_error = max(max_abs_error, local_max_abs);
            max_rel_error = max(max_rel_error, local_max_rel);
            rms_error += local_rms;
            max_analytical_value = max(max_analytical_value, local_max_analytical);
        }
    }
...
int main() {
    ...
    double first_start = omp_get_wtime();
    compute_first_time_step_fast(u0, u1);
    double first_end = omp_get_wtime();
    ...
    double analytical_start = omp_get_wtime();
    ...
    double analytical_end = omp_get_wtime();
    ...
    cout << "Total time: " << (total_end - total_start) << " sec" << endl;
